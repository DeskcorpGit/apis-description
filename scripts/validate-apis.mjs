...
// Roda o mesmo processo de validação do CI localmente.
// Uso: npm run validate:apis
...
const SPECTRAL_ARGS = ["@stoplight/spectral-cli@6.11.1", "lint", "--fail-severity=error"];
...
// Garantir que .spectral.yaml existe
...
// Verificar se pasta apis/ existe
if (!existsSync(APIS_DIR)) {
  console.log(yellow("  ⚠ Pasta apis/ não encontrada. Nenhuma especificação para validar.\n"));
  process.exit(0);
}

// Buscar arquivos YAML/JSON recursivamente
...
for (const file of files) {
  ...
  if (result.status === 0) {
    ...
    hasError = true;
    // Exibir apenas as linhas de erro (filtrar warnings)
...
if (hasError) {
  console.log(red(bold("❌ Validação falhou. Corrija os erros antes de fazer o push.\n")));
  process.exit(1);
} else {
